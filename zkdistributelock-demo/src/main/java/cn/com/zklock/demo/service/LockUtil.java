...
public class LockUtil {
    ...
    public synchronized void init(String connect) {
        if (client != null) {
            return;
        }
        RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
        client = CuratorFrameworkFactory.builder().connectString(connect).sessionTimeoutMs(10000).retryPolicy(retryPolicy)
        ...
        try {
            ...
            if (client.checkExists().forPath("/ShardLockDemo") == null) {
                client.create().creatingParentsIfNeeded()
                        .withMode(CreateMode.PERSISTENT)
                        .withACL(ZooDefs.Ids.OPEN_ACL_UNSAFE).forPath("/ShardLockDemo");
            }
        } catch (Exception e) {
